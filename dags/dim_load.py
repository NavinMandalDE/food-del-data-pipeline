...
from airflow.providers.postgres.operators.postgres import PostgresOperator # Using Postgres Operator as Redshift uses Postgres Engine
...
with DAG(
    ...
    create_schema = PostgresOperator(
        ...
        sql="CREATE SCHEMA IF NOT EXISTS food_delivery;",
    )
    ...
    drop_dimCustomers = PostgresOperator(
        ...
        postgres_conn_id='redshift_conn_id',
        sql="DROP TABLE IF EXISTS food_delivery.dimCustomers;",
    )

    drop_dimRestaurants = PostgresOperator(
        task_id='drop_dimRestaurants',
        postgres_conn_id='redshift_conn_id',
        sql="DROP TABLE IF EXISTS food_delivery.dimRestaurants;",
    )

    drop_dimDeliveryRiders = PostgresOperator(
        task_id='drop_dimDeliveryRiders',
        postgres_conn_id='redshift_conn_id',
        sql="DROP TABLE IF EXISTS food_delivery.dimDeliveryRiders;",
    )

    drop_factOrders = PostgresOperator(
        task_id='drop_factOrders',
        postgres_conn_id='redshift_conn_id',
        sql="DROP TABLE IF EXISTS food_delivery.factOrders;",
    )
    ...
    create_dimCustomers = PostgresOperator(
        ...
        postgres_conn_id='redshift_conn_id',
        sql="""
            CREATE TABLE food_delivery.dimCustomers (
    ...
    create_dimRestaurants = PostgresOperator(
        ...
        postgres_conn_id='redshift_conn_id',
        sql="""
            CREATE TABLE food_delivery.dimRestaurants (
    ...
    create_dimDeliveryRiders = PostgresOperator(
        ...
        postgres_conn_id='redshift_conn_id',
        sql="""
            CREATE TABLE food_delivery.dimDeliveryRiders (
    ...
    create_factOrders = PostgresOperator(
        ...
        postgres_conn_id='redshift_conn_id',
        sql="""
            CREATE TABLE food_delivery.factOrders (
                OrderID INT PRIMARY KEY,
                CustomerID INT REFERENCES food_delivery.dimCustomers(CustomerID),
                RestaurantID INT REFERENCES food_delivery.dimRestaurants(RestaurantID),
                RiderID INT REFERENCES food_delivery.dimDeliveryRiders(RiderID),
